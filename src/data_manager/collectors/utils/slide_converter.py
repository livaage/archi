...
class SlideConverter:
    ...
    def convert(self, file_path: Union[str, Path], content_type: Optional[str] = None) -> ConversionResult:
        ...
        try:
            ...
            logger.info(f"Converted {file_path.name} to markdown ({len(markdown)} chars)")
    ...
    def convert_bytes(
        ...
        try:
            ...
            try:
                logger.info(f"Converting bytes to markdown (type: {content_type}, size: {len(file_bytes)} bytes)")
    ...
    def register_plot_hook(self, hook: Callable) -> None:
        ...
        logger.info(f"Registered plot hook: {hook.__name__ if hasattr(hook, '__name__') else 'unnamed'}")

    def _get_extension_from_content_type(self, content_type: str, filename: Optional[str] = None) -> str:
